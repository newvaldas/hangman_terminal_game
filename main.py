...
def select_letter(letters) -> list:
        index=-1
        while(index==-1):
...
def update_mask(masked_word,word,letter) -> list:
        found=0
        new_masked_word=''
...
def reduce_health(health,found) -> int:
        if(found==0):
            health-=1
        return health

def print_letters(letters) -> str:
...
def print_info(masked_word,health) -> str:
        print("Health: "+"❤"*health)
